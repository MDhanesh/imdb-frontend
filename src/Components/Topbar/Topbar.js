...
function Topbar() {
  ...
  const getUser = async () => {
    try {
      const { data } = await axios.get(
        "https://imdbwebapi-jse5.onrender.com/user",
        {
          headers: {
  ...
  return (
    <div className="topbar-container">
      ...
      <h4 className="topbar-name">Internet Movie DataBase</h4>
      <h5 className="user-details">
        <span className="topbar-userdetails" style={{ color: "skyblue" }}>
          <i class="fa-regular fa-user"></i>
        </span>
        {user.name}
      </h5>
      <button onClick={logout} className="btn btn-danger btn-sm topbar-btn">
        Log Out
      </button>
    </div>
  );
